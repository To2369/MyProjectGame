...
DebugRenderer::DebugRenderer(ID3D11Device* device)
{
	...
	// 頂点シェーダー
	{
		ShaderManager::Instance()->CreateVsFromCso(device, ".\\Data\\Shader\\DebugVS.cso", vertexShader.GetAddressOf(),
			inputLayout.GetAddressOf(), inputElementDesc, ARRAYSIZE(inputElementDesc));
	}

	// ピクセルシェーダー
	{
		ShaderManager::Instance()->CreatePsFromCso(device, ".\\Data\\Shader\\DebugPS.cso", pixelShader.GetAddressOf());
	}
